...
document.addEventListener('DOMContentLoaded', async function load_Map(){
    ...
    var contacts = await fetch("/api/contacts"); // change to api functionality
    ...
    var zones = await fetch("/api/zones"); // change to api functionality
...
function show_marker(markerid){
    ...
    // var contact = current_Contacts.get(Number(markerid.split("_")[0]));
    // if (contact.status != null){
    //     markers_.get(markerid).getElement().style.backgroundColor = status_icon_colormap.get(contact.status);
    // } else {
    //     markers_.get(markerid).getElement().style.backgroundColor = status_icon_colormap.get("unknown");
    // }
    ...
    marker_contact_.on('mouseover', function(){
        marker_contact_.openPopup();
        show_dets_marker(marker_contact_);
...
function plot_this_contact(contact, new_marker_=true){
    if(contact.current_loc != null){
        if (new_marker_){
            ...
            // if (contact.status != null){
            //     marker_this_contact_.getElement().style.backgroundColor = status_icon_colormap.get(contact.status);
            // } else{
            //     marker_this_contact_.getElement().style.backgroundColor = status_icon_colormap.get("unknown");
            // }
...
function show_dets_marker(contact){
    ...
    var this_marker_properties_element_ = document.getElementById('current_contact_info_card_');
    document.getElementById("current_contact_desig_show_").textContent = contact.designator;
    var this_contact_info_elem_ = document.getElementById("current_contact_add_info_");
    this_contact_info_elem_.style.overflowX = "auto";
    if(contact.meta != null){
        this_contact_info_elem_.textContent = contact.meta;
    }
    if(contact.last_report_time != null){
        document.getElementById('last_repd_datetime_').textContent = "REPORTED: " +  String(contact.last_report_time);
    }
    var showtext = "";
    ...
    if (contact.status != null){
        showtext += "<div style='background-color:" + status_icon_colormap.get(contact.status) + ";'>" + capitalize(String(contact.status)) + " </div> <br>";
        // document.getElementById('color_node_status_').style.backgroundColor = status_icon_colormap.get(contact.status);
        // document.getElementById('color_node_status_').style.width = '5%';
    }
    if (contact.current_loc != null){
    ...
    this_marker_properties_element_.style.color = 'antiquewhite';
}
...
function show_dets_zone(zone){
    ...
    if (zone.type != null){
        showtext += "<br>TYPE: " + String(zone.type) + "<br><br>";
    }
    if (zone.desc != null){
